...
export default function MainItemLayout({ title, itemCount, items }: { title: string, itemCount: number, items: Array<{id: number, sort: string, name: string, price: number, image: string}> }) {
    return (
        <div className="mb-16">
            <div className="flex py-3 justify-between mb-4 border-b items-center">
                <div className="flex gap-4 items-center">
                <span className="font-semibold text-xl">{title}</span>
                <span className="text-sm text-gray-400">상품수 {itemCount}개</span>
                </div>
