...
//             onMouseDown={() => {
//                 setActiveId(node.id)
//                 setActiveCategory(node.title)
//             }}
//         >
//             <span {...listeners} {...attributes} className="cursor-grab mr-2 flex-shrink-0">
//                 <GripVertical className="w-4 h-4 text-gray-400" />
//             </span>
//             <span className="font-semibold text-[#5E5955] flex-grow text-left" style={{ marginLeft: depth * 20 }}>{t(node.title)}</span>
//         </div>
//     )
// }
...
//         try {
//             console.log('product category post token: ', session.token)
//             const response = await api.post('/product/category', {
//                 idx: 1,
//                 catename: newCategoryName,
//                 catecode: newCategoryName,
//                 upcate: activeCategory,
//                 isshow: 'Y',
//                 orders: 1
//             }, {
//                 token: session.token
...
//         try {
//             console.log('product category delete token: ', session.token)
//             // const response = await api.delete(`/product/cate/${activeId}`, {}, {
//             const response = await api.delete(`/product/cate/${1}`, {}, {
//                 token: session.token
//             })
//             console.log('response: ', response)
//         } catch (error) {
...
//             //     }
//             // };
//             // findParentRecursive(tree, activeId, null);
//             // console.log('ParentId: ', parentId)
            
//             setIsAddingCategory(true)
//             setNewCategoryParentId(activeId)
//             // setNewCategoryParentId(parentId)
//         }
//     }

//     const addCategoryAtLevel = (
//         nodes: ProductCategoryNode[],
//         parentId: string | null,
//         newCategory: ProductCategoryNode
//     ): ProductCategoryNode[] => {
//         if (parentId === null) {
//             // 최상위 레벨에 추가
//             return [...nodes, newCategory]
//         }
    
//         // 하위 레벨에 추가
//         return nodes.map(node => {
//             if (node.id === parentId) {
//                 // 해당 부모 노드의 children에 추가
//                 return {
//                     ...node,
...
//                 return {
//                     ...node,
//                     children: addCategoryAtLevel(node.children, parentId, newCategory)
//                 }
//             }
//             return node
//         })
...
//         const newCategory: ProductCategoryNode = {
...
//         console.log('parent category name: ', activeCategory)

//         setTree(prev => addCategoryAtLevel(prev, newCategoryParentId, newCategory))
//         setActiveId(newCategory.id)
//         setActiveCategory(newCategory.title)
...
//                     <div key={node.id}>
//                         <SortableItem node={node} depth={depth} activeId={activeId} setActiveId={setActiveId} setActiveCategory={setActiveCategory} t={t} />
//                         {node.children && (
//                             <RenderTree nodes={node.children} depth={depth + 1} activeId={activeId} setActiveId={setActiveId} t={t} isAddingCategory={isAddingCategory}
...
//                         {isAddingCategory && node.id === newCategoryParentId && (
...
export default function CategorySettingModal({ open, setOpen, t }: { open: boolean, setOpen: (v: boolean) => void, t: (key: string) => string }) {
    ...
    const handleSave = async () => {
        ...
        if (isAddingMode) {
            ...
            await api.post("/product/category",
                addedForm, {
                    token: session.token
                }
            )
    ...
    function handleDragEnd(event: DragEndEvent) {
        ...
        //             collectOrders(newTree)
        //         } else {
        //             const parentNode = findNodeAndParent(newTree, overParentId).node
        //             if(parentNode?.children) {
        //                 collectOrders(parentNode.children, overParentId)
        //             }
        //         }
        ...
        // updateOrders()
        // setTree((prev) => moveWithinSiblings(prev, overParentId, active.id as number, over.id as number))
    }
    ...
    const handleDeleteCategory = async () => {
        ...
        if (!selectedCategory || status !== 'authenticated' || !session?.token) return;
